...
const outputFolder = path.resolve(
  process.argv[3] ?? path.join(path.dirname(inputFolder), `${path.basename(inputFolder)}-contact-sheets`)
);
...
const orientationOf = (width, height) =>
  width > height * 1.08 ? "landscape" : height > width * 1.08 ? "portrait" : "square";
...
for (let page = 0; page < pages; page += 1) {
  ...
  for (let index = 0; index < pageFiles.length; index += 1) {
    ...
    const thumbnail = await sharp(file)
      .rotate()
      .resize(280, 175, { fit: "contain", background: "#111111" })
      .png()
      .toBuffer();
    ...
    const labelSvg = Buffer.from(
      `<svg width="280" height="55" xmlns="http://www.w3.org/2000/svg"><rect width="280" height="55" fill="#f5f0e8"/><text x="8" y="18" font-family="Arial, sans-serif" font-size="11" fill="#171717">${escapeXml(label.slice(0, 76))}</text><text x="8" y="37" font-family="Arial, sans-serif" font-size="10" fill="#6b625a">${escapeXml(label.slice(76, 152))}</text></svg>`
    );
    composites.push({ input: thumbnail, left: x, top: y }, { input: labelSvg, left: x, top: y + 175 });
  }
  const pageNumber = String(page + 1).padStart(2, "0");
  await sharp({
    create: { width: columns * cellWidth, height: rows * cellHeight, channels: 3, background: "#f5f0e8" }
  })
