...
for (const asset of inventory) {
  ...
  const matched = sources.filter((source) => source.text.includes(publicPath) || source.text.includes(basename));
  placements.push({
    ...
    evidenceStrength: Number(((asset.scores.commercialRelevance + asset.scores.internationalContext) / 2).toFixed(1)),
...
const homepageSelectedIds = [
  "changan-europe-launch-2025-04-gallery", "catl-open-day-2025-06-gallery", "london-automotive-brand-film-03-gallery",
  "byd-bd11-london-07-gallery", "changan-europe-launch-2025-01-hero", "catl-open-day-2025-05-gallery", "leapmotor-iaa-2023-07-gallery",
  "london-automotive-brand-film-06-gallery", "byd-bd11-london-02-cover", "changan-europe-launch-2025-02-cover"
];
const selectedPaths = new Set(portfolio.filter((item) => homepageSelectedIds.includes(item.id)).map((item) => item.publicPath));
const selected = placements.filter((asset) => selectedPaths.has(asset.publicPath));
const selectedByHash = new Map();
for (const asset of inventory.filter((item) => selected.some((selectedAsset) => selectedAsset.source === item.relativePath))) {
...
console.log(`Phase 5 media usage audit: ${placements.length} assets, ${selected.length} homepage selections, ${repeatedHomepageBinaries.length} repeated binaries.`);
