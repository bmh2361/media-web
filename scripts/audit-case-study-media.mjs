...
const manifest = JSON.parse(
  await readFile(path.join(root, "content", "portfolio-media.generated.json"), "utf8")
);
const active = [
...
const markdown = [
  ...
  "",
  `Generated for ${active.length} active projects.`,
  ""
];
...
for (const slug of active) {
  ...
  for (const [index, record] of records.entries()) {
    ...
    const image = await sharp(path.join(root, "public", record.publicPath.slice(1)))
      .resize({ width: 380, height: 270, fit: "contain", background: "#101010" })
      .png()
      .toBuffer();
  ...
  await sharp({
    create: { width: columns * cellWidth, height: rows * cellHeight, channels: 3, background: "#101010" }
  })
    .composite(composites)
    .png()
    .toFile(path.join(output, `${slug}.png`));
  markdown.push(
    `## ${slug}`,
    "",
  ...
  for (const record of records) {
    ...
    const orientation =
      ratio > 1.8 ? "panoramic" : ratio > 1.08 ? "landscape" : ratio < 0.92 ? "portrait" : "square";
    markdown.push(
      `| ${record.sourceFile} | ${record.width}×${record.height} | ${ratio.toFixed(3)} | ${orientation} | ${record.altEn} |`
    );
  }
  markdown.push("");
}
...
console.log(
  `Audited ${active.reduce((total, slug) => total + manifest.records.filter((record) => record.projectId === slug).length, 0)} active source records and generated ${active.length} contact sheets.`
);
