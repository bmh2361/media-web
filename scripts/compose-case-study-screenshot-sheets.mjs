...
async function sheet(name, selected, columns, width, height) {
  ...
  for (const [index, file] of selected.entries()) {
    const image = await sharp(path.join(directory, file))
      .resize({ width, height, fit: "contain", background: "#111" })
      .png()
      .toBuffer();
...
await sheet(
  "heroes-desktop-contact-sheet.png",
  files.filter((file) => file.endsWith("-1440-hero.png")).sort(),
  3,
  480,
  380
);
await sheet(
  "heroes-mobile-contact-sheet.png",
  files.filter((file) => file.endsWith("-390-hero.png")).sort(),
  4,
  280,
  480
);
await sheet(
  ...
  files.filter((file) => file.startsWith("preview-")).sort(),
  3,
  480,
  620
);
await sheet(
  "gallery-first-last-contact-sheet.png",
  files.filter((file) => /-1440-(first|last)\.png$/.test(file)).sort(),
  4,
  360,
  300
);
