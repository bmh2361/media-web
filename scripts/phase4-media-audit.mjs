...
const normalizedVariant = (relative) =>
  relative
    .toLowerCase()
    .replace(/\.(avif|webp|jpe?g|png|gif|svg)$/i, "")
    .replace(/-(desktop|tablet|mobile|thumb|thumbnail|hero|cover|gallery|detail|original|\d+x\d+)$/i, "");
...
const sourceFiles = (
  await Promise.all(
    sourceRoots.map(async (dir) => {
      try {
        return await walkText(path.join(root, dir));
      } catch {
        return [];
      }
    })
...
for (const file of files) {
  ...
  const luminance = stats
    ...
    : 128;
  const contrast = stats
    ? stats.channels.slice(0, 3).reduce((sum, channel) => sum + channel.stdev, 0) / 3
    : 0;
  const semantic = relative.toLowerCase();
  const originalLike =
    /(^|\/)(portfolio|expertise)\//.test(semantic) && !/(mobile|thumbnail|art-directed)/.test(semantic);
  const commercial = clamp(
    1.8 +
      (/portfolio|expertise|event|launch|interview|technology|talent|creator/.test(semantic) ? 1.6 : 0) +
      (originalLike ? 0.8 : 0)
  );
  const visual = clamp(
    1.5 +
      (width >= 1200 ? 1.6 : width >= 900 ? 1.1 : 0.4) +
      (contrast >= 30 ? 1.1 : contrast >= 18 ? 0.6 : 0.2)
  );
  const premium = clamp((visual + commercial) / 2 + (/art-directed|portfolio/.test(semantic) ? 0.5 : 0));
  const international = clamp(
    1.2 +
      (/london|uk|europe|munich|iaa|changan|byd|catl|leapmotor/.test(semantic)
        ? 2.7
        : /event|exhibition/.test(semantic)
          ? 1.4
          : 0)
  );
  const focal = clamp(
    2.3 +
      (/portrait|interview|creator|artist|robot|vehicle|automotive/.test(semantic) ? 1.2 : 0) +
      (width >= 900 ? 0.6 : 0)
  );
  const crop = clamp(
    2.2 +
      (orientation !== "square" ? 0.8 : 0.4) +
      (width >= 900 ? 0.8 : 0) -
      (/mobile/.test(semantic) ? 0.2 : 0)
  );
  const colour = clamp(3.1 + (luminance > 45 && luminance < 215 ? 0.8 : 0.2) + (contrast > 18 ? 0.5 : 0));
  records.push({
    filename: path.basename(file),
    relativePath: relative,
    directory: posix(path.dirname(relative)),
    width,
    height,
    aspectRatio: height ? Number((width / height).toFixed(4)) : null,
    orientation,
    bytes: input.byteLength,
    format: metadata.format ?? path.extname(file).slice(1),
    sha256: crypto.createHash("sha256").update(input).digest("hex"),
    variantGroup: normalizedVariant(relative),
    likelyProject: relative.split("/").slice(0, 4).join("/"),
    currentWebsiteUsage: sourceText.includes(`/media/${relative}`),
    averageLuminance: Number(luminance.toFixed(1)),
    scores: {
      ...
      focalClarity: focal,
      compositionFlexibility: crop,
      websiteCropSafety: crop,
      colourCompatibility: colour
    },
    totalScore: Number(
      (commercial + visual + premium + international + focal + crop + crop + colour).toFixed(1)
    )
  });
}
...
for (const record of records)
  hashes.set(record.sha256, [...(hashes.get(record.sha256) ?? []), record.relativePath]);
for (const record of records)
  record.duplicates = hashes.get(record.sha256).filter((item) => item !== record.relativePath);
...
async function sheet(name, items) {
  ...
  if (!rows) return;
  const canvas = sharp({
    create: { width: cellW * columns, height: cellH * rows, channels: 3, background: "#111214" }
  });
  const composites = [];
  for (let index = 0; index < items.length; index++) {
    ...
    try {
      const thumb = await sharp(path.join(mediaRoot, record.relativePath), {
        animated: false,
        failOn: "none"
      })
        .rotate()
        .resize(cellW - 12, 142, { fit: "contain", background: "#25272a" })
        .jpeg({ quality: 72 })
        .toBuffer();
  ...
  await canvas
    ...
    .jpeg({ quality: 82 })
    .toFile(path.join(sheetRoot, `${name}.jpg`));
}
...
for (const [directory, items] of groups)
  await sheet(`folder--${directory.replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "") || "root"}`, items);
for (const orientation of ["landscape", "portrait", "square"]) {
  const oriented = records.filter((record) => record.orientation === orientation);
  for (let index = 0; index < oriented.length; index += 100)
    await sheet(
      `orientation--${orientation}--${String(index / 100 + 1).padStart(2, "0")}`,
      oriented.slice(index, index + 100)
    );
}
await sheet(
  "homepage-candidates",
  records
    .filter((record) => record.totalScore >= 30)
...
const summary = {
  ...
  root: posix(mediaRoot),
  totalAssets: records.length,
  formats: Object.fromEntries(
    [...new Set(records.map((record) => record.format))]
      .sort()
      .map((format) => [format, records.filter((record) => record.format === format).length])
  ),
  orientations: Object.fromEntries(
    ["landscape", "portrait", "square"].map((value) => [
      value,
      records.filter((record) => record.orientation === value).length
    ])
  ),
  ...
  topHomepageCandidates: records
    .sort((a, b) => b.totalScore - a.totalScore)
    .slice(0, 40)
    .map(({ relativePath, totalScore, scores }) => ({ relativePath, totalScore, scores }))
};
