  ...
  return (
    <div className="relative grid gap-8 lg:grid-cols-3">
      {process.map((step, index) => (
        <article key={step.title.en} className="relative rounded-lg border border-ink/10 bg-pearl p-7 shadow-soft">
          ...
          <p className="mt-5 text-sm leading-6 text-ink/62">{step.text[language]}</p>
        </article>
      ))}
    </div>
  );
