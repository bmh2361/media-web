...
const slugs = ["wang-linkai-london-concert", "geely-london-brand-launch", "changan-europe-launch-2025", "catl-open-day-2025", "yue-yunpeng-london-live", "london-fashion-week-2025", "leapmotor-iaa-2023", "byd-bd11-london", "agibot-london-launch", "london-automotive-brand-film", "beauty-fashion-brand-content", "european-road-lifestyle"];
...
test("capture index preview art direction at required widths", async ({ page }, testInfo) => {
  ...
  await page.goto("/en/work");
  for (const slug of ["wang-linkai-london-concert", "geely-london-brand-launch", "changan-europe-launch-2025", "catl-open-day-2025", "london-fashion-week-2025", "agibot-london-launch"]) {
...
test("capture every active project hero and media sequence on desktop and mobile", async ({ page }, testInfo) => {
  test.skip(testInfo.project.name === "mobile");
  mkdirSync(output, { recursive: true });
  await page.addInitScript(() => sessionStorage.setItem("vbm-opening-seen", "1"));
  for (const width of [1440, 390]) {
    ...
    for (const slug of slugs) {
      ...
      await page.locator('[data-case-section="hero"]').screenshot({ path: path.join(output, `${slug}-${width}-hero.png`) });
